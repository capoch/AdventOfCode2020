...
public class SeatingSystem {
    ...
    public static int stableStateCount(boolean lignOfSight) {
        while(!SeatingSystem.stable) { SeatingSystem.transform(lignOfSight);}
    ...
    public static void transform(boolean lignOfSight) {
        ...
        for(int j = 0; j < seatPlan.size(); j++) {
            ...
            for(int i = 0; i < seatPlan.get(j).size(); i++) {
                line.add(willChange(i, j, lignOfSight));
            }
        ...
        if(seatPlan.equals(newSeatPlan)) { stable = true; }
        seatPlan = newSeatPlan;
    }

    private static boolean willChange(int x, int y, boolean lineOfSight) {
        if(lineOfSight) {
    ...
    private static String getSeat(int x, int y) { return seatPlan.get(y).get(x); }
    ...
    public static void main(String[] args) throws IOException {
        ...
        //System.out.println("Part One: " + stableStateCount(false));
        getSeatPlan("resources/input11.txt");
        System.out.println("Part Two: " + stableStateCount(true));
    }
}